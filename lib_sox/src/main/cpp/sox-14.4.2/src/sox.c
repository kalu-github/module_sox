...
static enum {
    sox_sox, sox_play, sox_rec, sox_soxi
} sox_mode;
...
typedef enum {
    RG_off, RG_track, RG_album, RG_default
} rg_mode;
...
static void cleanup(void) {
    ...
    if (file_count) {
        if (ofile->ft) {
            if (!success &&
                ofile->ft->io_type == lsx_io_file) {   /* If we failed part way through */
...
static void atexit_cleanup(void) {
     ...
     */
    lsx_warn("Your LibSox has been exited");
    if (!cleanup_called)
        cleanup();
}
...
static void play_file_info(sox_format_t *ft, file_t *f, sox_bool full) {
    ...
    if (f && f->replay_gain != HUGE_VAL) {
        sprintf(buffer, "%s gain: %+.1fdB",
                lsx_find_enum_value(f->replay_gain_mode, rg_modes)->text, f->replay_gain);
        buffer[0] += 'A' - 'a';
                    ...
                    *p = 0;
                    for (i = 0; i < input_count; ++i)
                        if (ws < z->ilen[i] && s < files[i]->ft->signal.channels) {
                            ...
                            double sample = *p +
                                            (double) z->ibuf[i][ws * files[i]->ft->signal.channels +
                                                                s];
...
static void parse_effects(int argc, char **argv) {
    while (optstate.ind < argc) {
        ...
        optstate.ind++;
        for (j = 0;
             j < (size_t) (argc - optstate.ind) && !sox_find_effect(argv[optstate.ind + j]) &&
...
static void add_effects(sox_effects_chain_t *chain) {
    ...
    for (i = 0; i < chain->length; ++i) {
        ...
        lsx_report(format, effp->handler.name, effp->out_signal.rate,
                   ...
                   str_time(effp->out_signal.length / effp->out_signal.channels /
...
static void calculate_output_signal_parameters(void) {
    ...
    for (i = 0; i < nuser_effects[current_eff_chain] && !ofile->signal.channels; ++i)
        ofile->signal.channels = user_efftab[nuser_effects[current_eff_chain] - 1 -
                                             i]->out_signal.channels;
    ...
    ofile->signal.length = (uint64_t) (
            olen * ofile->signal.channels * ofile->signal.rate / combiner_signal.rate + .5);
}
...
static int process(void) {         /* Input(s) -> Balancing -> Combiner -> Effects -> Output */
    int flow_status;
...
static void display_supported_effects(void) {
    ...
    for (i = 0; sox_effect_fns[i]; i++) {
        ...
        if (e && e->name)
            printf(" %s%s", e->name,
                   (e->flags & SOX_EFF_DEPRECATED) ? "*" : (e->flags & SOX_EFF_ALPHA) ? "+"
                                                                                      : (e->flags &
                                                                                         SOX_EFF_INTERNAL)
                                                                                        ? "#" : "");
    }
...
enum {
    ENDIAN_little, ENDIAN_big, ENDIAN_swap
};
...
#define SOX_BUFMIN 16
                    ...
                    case 2:
                        combine_method = enum_option(optstate.arg, optstate.lngind,
                                                     combine_methods);
                        break;
                    ...
                    case 10:
                        ...
                        break;
                    case 11:
                        sox_effects_globals.plot = enum_option(optstate.arg, optstate.lngind,
                                                               plot_methods);
                        break;
                    case 12:
            ...
            case 'V':
                if (optstate.arg == NULL)
                    ...
                    if (sscanf(optstate.arg, "%d %c", &i, &dummy) != 1 || i < 0) {
                        ...
                        lsx_fail("Verbosity value `%s' is not a non-negative integer",
                                 optstate.arg);
                        exit(1);
                    }
...
static void parse_options_and_filenames(int argc, char **argv) {
    ...
    if (env_opts && *env_opts) {
        ...
        lsx_getopt_init(argc2, argv2, getoptstr, long_options, lsx_getopt_flag_opterr, 1,
                        &optstate);
    ...
    for (; optstate.ind < argc && !sox_find_effect(argv[optstate.ind]); init_file(&opts)) {
        ...
        else if (sox_parse_playlist((sox_playlist_callback_t) add_file, &opts,
                                    argv[optstate.ind++]) != SOX_SUCCESS)
            exit(1);
    }
    if (env_opts && *env_opts) {
...
static int soxi1(soxi_t const *type, char const *filename) {
    ...
    switch (*type) {
        ...
        case Channels:
            ...
            break;
        case Samples:
            if (soxi_total == -1) printf("%" PRIu64 "\n", ws);
            break;
        case Duration:
            if (soxi_total == -1) printf("%s\n", str_time(secs));
            break;
        case Duration_secs:
            if (soxi_total == -1) printf("%f\n", secs);
            break;
        case Bits:
...
static int soxi(int argc, char *const *argv) {
    ...
    for (; optstate.ind < argc; ++optstate.ind) {
        if (sox_is_playlist(argv[optstate.ind]))
            num_errors += (sox_parse_playlist((sox_playlist_callback_t) soxi1, &type,
                                              argv[optstate.ind]) != SOX_SUCCESS);
...
void InitParam() {
    ...
    optstate.argc = 0;
    optstate.argv = NULL;
    optstate.curpos = NULL;
    optstate.flags = 0;
    optstate.ind = 0;
    optstate.lngind = 0;
    optstate.longopts = NULL;
    optstate.opt = 0;
    optstate.shortopts = NULL;
    combine_method = sox_default;
    output_method = sox_single;

    no_clobber = sox_false, interactive = sox_false;
    ...
    original_termios_saved = sox_false;
}
...
int main(int argc, char **argv) {
    ...
    char mybase[6];
    char *argvTemp[512];
    memset(argvTemp, 0, sizeof(char *) * 512);
//    sox_globals.tmp_path = lsx_strdup("/sdcard/vtech");
    for (int b = 0; b < argc; b++) {
        argvTemp[b] = (char *) malloc(strlen(argv[b]) + 1);
        strcpy(argvTemp[b], argv[b]);
    }
    ...
    parse_options_and_filenames(argc, argvTemp);//argc,argv
